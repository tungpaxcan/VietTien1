        ...
        success: function (data) {
            ...
            if (data.code == 200) {
                $.each(data.c, function (k, v) {
                    ...
                    table += '<a class="nav-link" href="/WareHouse/Edits/' + v.id + '">';
...
//----------------Add::WareHouses---------------------
function Add() {
    var id ="K"+ $('#id').val().trim();
    var name = $('#name').val().trim();
    $('.Loading').css("display", "block");
    if (name.length <= 0) {
        alert("Nhập Tên Kho Hàng !!!")
        return;
    }  if (id.length <= 0) {
        alert("Nhập Mã K !!!")
        return;
