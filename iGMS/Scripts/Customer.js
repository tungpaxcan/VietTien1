...
function Customer(pagenum, page, seach) {
    $.ajax({
        url: '/customer/List',
        type: 'get',
        ...
        success: function (data) {
            ...
            if (data.code == 200) {
                $.each(data.c, function (k, v) {
                    ...
                    table += '<a class="nav-link" href="/Customer/Details/' + v.id + '">';
                    table += '<i class="nav-icon icon-xl la la-building"></i>';
                    table += '<span class="nav-text">Detail</span></a></li>';
                    table += ' <li class="nav-item">';
                    table += '<a class="nav-link" href="/Customer/Edits/' + v.id + '">';
...
function Add() {
    ...
    if (id.length<=0) {
        ...
        alert("Nhập Người Đại Diện")
        return;
    } if (position.length <= 0) {
        alert("Nhập Chức Vụ")
        return;
    } if (stk.length <= 0) {
        alert("Nhập Tài Khoản Giao Dịch !!")
        return;
    } if (bank.length <= 0) {
    ...
    $.ajax({
        ...
        type: 'post',
        data: {
            id, name, address, nametransaction: nametransaction, taxcode, fax: fax, phone, email,
            represent, position, website: website, stk, bank, groupgoods, debtfrom: debtfrom, debtto: debtto
