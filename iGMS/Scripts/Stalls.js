...
var page = 1;
var seach = "";
var idstore = $('#idstore').val().trim();
Stalls(pagenum, page, seach);
...
function Stalls(pagenum, page, seach) {
    var id = $('#idstore').val().trim();
    $.ajax({
        url: '/stalls/List',
        type: 'get',
        ...
        success: function (data) {
            ...
            if (data.code == 200) {
                $.each(data.c, function (k, v) {
                    ...
                    table += '<a class="nav-link" href="/Stalls/Edits/' + v.id + '">';
...
function Add() {
    ...
    var id = "QB" + $('#id').val().trim();
    $('.Loading').css("display", "block");
 if (id.length <= 0) {
        alert("Nhập Mã Quầy Bán  !!!")
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code == 200) {
                ...
                window.location.href = "/stalls/Index/" + idstore;
