...
namespace iGMS.Controllers
{
    public class SizeController : BaseController
    {
        ...
        public ActionResult Edits(string id)
        {
            ...
            iGMS.Models.Size size = db.Sizes.Find(id);
            if (size == null)
            {
                return HttpNotFound();
            }
        ...
        public JsonResult Add(string name, string id,string des,bool status)
        {
            try
            {
                ...
                db.Sizes.Add(d);
                db.SaveChanges();
                return Json(new { code = 200, msg = "Thêm Kích Thước Thành Công" }, JsonRequestBehavior.AllowGet);

            }
            catch (Exception e)
            {
                return Json(new { code = 500, msg = "Thêm Kích Thước Thất Bại" + e.Message }, JsonRequestBehavior.AllowGet);
                }
        }
        [HttpPost]
        public JsonResult Edit(string id, string name, string des, bool status)
        {
            try
            {
                ...
                d.ModifyBy = nameAdmin;
                db.SaveChanges();
                return Json(new { code = 200, msg = "Sửa Kích Thước Thành Công" }, JsonRequestBehavior.AllowGet);

            }
            catch (Exception e)
            {
                return Json(new { code = 500, msg = "Sửa Kích Thước Thất Bại" + e.Message }, JsonRequestBehavior.AllowGet);
            }
        }
        [HttpPost]
        public JsonResult Delete(string id)
        {
            try
            {
                var d = db.Sizes.Find(id);
                Dele.DeleteSizes(id);
                return Json(new { code = 200, msg = "Xóa Kích Thước Thành Công" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception)
            {
                return Json(new { code = 500, msg = "Xóa Kích Thước Thất Bại" }, JsonRequestBehavior.AllowGet);
            }
        }
        [HttpPost]
