...
{
    public class MaterialAccountController : BaseController
    {
        ...
        public ActionResult Edits(string id)
        {
            ...
            MaterialAccount materialAccount = db.MaterialAccounts.Find(id);
            if (materialAccount == null)
            {
                return HttpNotFound();
            }
            return View(materialAccount);
        }
        [HttpGet]
        public JsonResult List(int pagenum, int page, string seach)
        {
            try
            {
                ...
                var a = (from b in db.MaterialAccounts.Where(x => x.Id.Length > 0)
                         select new
                         {
                             id = b.Id,
                             name = b.Name,
        ...
        [HttpGet]
        public JsonResult materialAccount()
        {
            try
            {
                var a = (from b in db.MaterialAccounts.Where(x => x.Id.Length > 0 && x.Status == true)
                         select new
                         {
                             id = b.Id,
                             name = b.Name,
        ...
        public JsonResult Add(string name, string id, string des, bool status)
        {
            try
            {
                var materialAccount = db.MaterialAccounts.Find(id);
                if (materialAccount == null)
                {
                    var session = (User)Session["user"];
                    var nameAdmin = session.Name;
                    var d = new MaterialAccount();
                    d.Name = name;
                    d.Id = id;
                    d.Description = des;
                    d.Status = status;
                    ...
                    db.MaterialAccounts.Add(d);
                    db.SaveChanges();
                    return Json(new { code = 200, msg = "Tạo TKVT Thành Công" }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return Json(new { code = 300, msg = "Mã TKVT Trùng (Đã Có Trong Hệ Thống)" }, JsonRequestBehavior.AllowGet);
                }

            }
            catch (Exception e)
            {
                return Json(new { code = 500, msg = "Tạo TKVT Thất Bại" + e.Message }, JsonRequestBehavior.AllowGet);
            }
        }
        [HttpPost]
        public JsonResult Edit(string name, string id, string des, bool status)
        {
            try
            {
                ...
                var session = (User)Session["user"];
                var nameAdmin = session.Name;
                var d = db.MaterialAccounts.Find(id);
                d.Name = name;
                d.Description = des;
                d.Status = status;
                d.ModifyDate = DateTime.Now;
        ...
        public JsonResult Delete(string id)
        {
            try
            {
                Dele.DeleteMaterialAccounts(id);
